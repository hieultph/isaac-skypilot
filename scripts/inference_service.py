...
pip install requests uvicorn fastapi json-numpy
...
Using HTTP
    Run server:
        ...
        python scripts/inference_service.py --server --http-server --port 8000 --api-token "your-secure-token"

        # Custom configuration
        python scripts/inference_service.py --server --http-server --host 0.0.0.0 --port 8000 --model-path "/workspace/isaac-skypilot/nvidia/uav_quadcopter_finetune"

    Run client:
        # Test HTTP client (replace URL with your bore URL)
        python scripts/inference_service.py --client --http-server --host your-bore-url --port 443

    Expose to Internet Using Bore
        bore local 8000 --to 159.223.171.199
...
class ArgsConfig:
    ...
    model_path: str = "nvidia/uav_quadcopter_finetune"
    """Path to the model checkpoint directory."""

    embodiment_tag: Literal[tuple(EMBODIMENT_TAG_MAPPING.keys())] = "uav_quadcopter"
    """The embodiment tag for the model."""

    data_config: str = "uav_quadcopter"
    """
...
def _example_http_client_call(obs: dict, host: str, port: int, api_token: str):
    ...
    time_start = time.time()
    response = requests.post(f"http://{host}:{port}/acti", json={"observation": obs})
