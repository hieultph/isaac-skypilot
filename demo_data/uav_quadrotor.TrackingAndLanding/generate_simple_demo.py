...
def create_sample_episode_data(episode_index, task_type, episode_length):
    """Create sample episode data as numpy arrays."""
    
    # Generate trajectory data
    traj_data = generate_uav_trajectory(task_type, episode_length)
    
    # Create observation state (18 dimensions)
    ...
    timestamps = np.arange(episode_length) / 30.0  # 30 FPS
    episode_indices = np.full(episode_length, episode_index)
    indices = np.arange(episode_length)
    task_indices = np.full(episode_length, get_task_index(task_type))
    validities = np.ones(episode_length, dtype=int)
    task_descriptions = np.full(episode_length, get_task_index(task_type))
    rewards = np.random.uniform(-0.1, 1.0, episode_length)
    dones = np.zeros(episode_length, dtype=bool)
    dones[-1] = True  # Last frame is done
    
    return {
        'observation_state': obs_state,
        'action': traj_data['actions'],
...
def save_episode_as_npy(episode_data, output_path):
    """Save episode data as numpy files for now (can be converted to parquet later)."""
    np.savez(output_path, **episode_data)
...
def generate_demo_data():
    ...
    for episode_idx, task_type, length in episodes:
        ...
        # Create episode data
        episode_data = create_sample_episode_data(episode_idx, task_type, length)
        
        # Save as numpy file (can be converted to parquet later)
        output_path = base_path / f"data/chunk-000/episode_{episode_idx:06d}.npz"
        save_episode_as_npy(episode_data, output_path)
    ...
    print("\nTo convert to parquet format, you'll need pandas:")
